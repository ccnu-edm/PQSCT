...
def evaluate_test(valid_data,valid_D,model):
    valid_model_pred = model.predict_generator(valid_D.__iter__(), steps=len(valid_D), verbose=1)#选用这个
    y_pred = mlb.transform(sigmoid_pre(mlb,valid_model_pred))
    y_true = mlb.transform(sigmoid_pre(mlb,valid_data[:, 1]))#选用这个
...
if __name__ == '__main__':
    model = build_bert(nclass=num_classes) 
    get_flops_params()
