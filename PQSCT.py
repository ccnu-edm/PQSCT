...
def seq_padding(X, padding=0, ML=maxlen, label=False):
    if label == False:
        ...
        while True: 
            ...
            for i in idxs:
                ...
                if len(X1) == self.batch_size or i == idxs[-1]:
                    ...
                    X3 = seq_padding(X3,padding=102)
                    X4 = seq_padding(X4,padding=1)
                    Y = seq_padding(Y,label=True)
                    yield [X1, X2,X3,X4], Y[:, 0, :]
                    [X1, X2, X3,X4,Y] = [], [], [],[],[]
...
def stra(strategy,x):
    if strategy=='mean':
        return MaskedGlobalAveragePooling1D()(x)
    elif strategy=='max':
        return MaskedGlobalMaxPool1D()(x)
    else:
        return Lambda(lambda x: x[:, 0])(x) 


def build_bert(nclass,batch_size=Batch_size,max_len=maxlen,strategy='max'):
    bert_model1 = load_trained_model_from_checkpoint(config_path, checkpoint_path, seq_len=None) 
    ...
    x_s = bert_model1([x1_in, x2_in]) #question
    x_r = bert_model2([x3_in, x4_in]) #resolve

    added = Add()([x_s, x_r])  
    added=Lambda(lambda x: x*0.5)(added) 
    subtracted_1 = Subtract()([x_s, x_r])
    subtracted_2 = Subtract()([x_r, x_s])

    x1=Lambda(lambda x: stra(strategy,x))(x_s) 
    x2=Lambda(lambda x: stra(strategy,x))(x_r) 
    x3=Lambda(lambda x: stra(strategy,x))(added) 
    x4=Lambda(lambda x: stra(strategy,x))(subtracted_1) 
    x5=Lambda(lambda x: stra(strategy,x))(subtracted_2) 
...
def write_t(h,p,r,f1,f1_macro,acc,strategy):
    with open(savepath+"result.txt", "a",encoding='utf-8') as f:
        f.write('\r\n')
        f.write("mean ,max or cls ? {}\n".format(strategy))
...
def run_kb(strategy):
    ...
    model = build_bert(nclass=num_classes,strategy=strategy) 
    ...
    write_t(test_h,test_p,test_r,test_f,test_f_macro,test_acc,strategy)
    model_path =savepath+'bert_'+str(Epoch)+'_'+str(strategy)+'.h5'
...
if __name__ == '__main__':
    ...
    for strategy in ['cls','mean','max']:
        test_h,test_p,test_r,test_f,test_f_macro,test_acc=run_kb(strategy)
